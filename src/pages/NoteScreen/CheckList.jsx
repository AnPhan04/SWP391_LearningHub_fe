...
function CheckList({id}) {
    ...
    useEffect(() => {
        fetchData(cardid); // Gọi API khi giá trị tham số thay đổi
        console.log("oke")
    }, [parameter]);
    ...
    async function fetchData(cardid, archive) {
        try {
            const response = await fetch(`http://localhost:8080/api/v1/checklists?cardid=${cardid}`, {
                method: "GET",
                credentials: "include"
            });
            ...
            const l1 = data.data.filter(list => list.checked === true).map(list =>
            ({
                id: list.id,
                name: list.name,
                checked: list.checked
            ...
            const check = l.filter(list => list.checked !== true).map(list => list.id);
            ...
            editContentRefs.current = Array(l.length).fill(null).map(() => React.createRef());
            editRefs.current = Array(l.length).fill(null).map(() => React.createRef())
        } catch (error) {
    ...
    async function fetchArchiveData(parameter) {
        try {
            const response = await fetch(`http://localhost:8080/api/v1/checklists/archive?id=${parameter}`, {
                method: "PUT",
                credentials: "include"
            });
            const data = await response.json();
    ...
    async function fetchDeleteData(parameter) {
        try {
            const response = await fetch(`http://localhost:8080/api/v1/checklists/checklist?cardid=${parameter}`, {
                method: "DELETE",
                credentials: "include"
            });
            const data = await response.json();
    ...
    async function fetchDeleteCheckListData(parameter) {
        try {
            const response = await fetch(`http://localhost:8080/api/v1/checklists?id=${parameter}`, {
                method: "DELETE",
                credentials: "include"
            });
            const data = await response.json();
    ...
    const handleSaveClick = (index, id) => {
        const editedName = sanitizeInput(editContentRefs.current[index].current.innerText);

        console.log('Edited title:', editedName);
    ...
    return (
        <Box sx={{width:'100%', marginTop: 2,padding: '10px', }}>
            ...
            <Box>
                <LinearProgressWithLabel value={progress} sx={{ height: "7px", borderRadius: "5px" }} />
            </Box>
            <Box ref={wrapperRef}>
                {lists.map((list, index) => (
                    <Box key={index} sx={{
                        display: "flex", justifyContent: 'space-around', position: 'relative', marginBottom: '8px', padding: '8px 0px',
                        cursor: 'pointer',
                        '&:hover': {
                            backgroundColor: '#E4E6EA',
                        },
                    ...
                    >
                        <Box>
                            <input type="checkbox" style={{ position: 'absolute', top: '20px', left: '10px' }}
                        ...
                        <Box sx={{ width: '80%' }} >
                            <Typography
                                ...
                                style={checked.includes(list.id) ?
                                    {
                                        wordWrap: 'break-word', width: '100%', padding: '5px',
                                        fontSize: 20, margin: 0, fontWeight: 'bold', textDecoration: 'line-through'
                                    } :
                                    { wordWrap: 'break-word', width: '100%', padding: '5px', fontSize: 20, margin: 0, fontWeight: 'bold' }
                                }
                                dangerouslySetInnerHTML={{ __html: list.name }}
                            />
                            {showButtons[index] && (<Box sx={{ marginTop: 1 }}>
                                <Button variant="contained"
                                    sx={{
                                        fontFamily: "-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Noto Sans,Ubuntu,Droid Sans,Helvetica Neue,sans-serif",
                                        fontSize: "14px",
